...
| **resuming work a previous session left unfinished** · "what is still open" · a deferred decision · an item deliberately parked | `PENDING-WORK-CONTEXT.md` |
...
| [PENDING-WORK-CONTEXT.md](PENDING-WORK-CONTEXT.md) | **The deferred-work ledger.** What is still open, what is already DONE so it is not redone, the exact file:line sites, and the decision blocking each item. Currently: the live password-storage defect (steps 2–3), invoice-settings phase 2, the `DOD_VENDOR_SETTINGS` table, and the ungated bulk-products smoke script. **Enter here when resuming anything a previous session left unfinished.** | 2026-09-25 |
