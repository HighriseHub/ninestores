...
(defmethod render-json ((r WarehouseResponseModel) (ctx domain-ctx))
  "Single warehouse → JSON alist (caller or a list method applies
   json:encode-json-to-string). Allowlist only — see the contract note."
  (list (cons "code"   (warehouse-code r))
        (cons "name"   (wname r))
        (cons "city"   (wcity r))
        (cons "state"  (wstate r))
        (cons "type"   (warehouse-type r))
        (cons "gstin"  (warehouse-gstin r))
        (cons "owner"  (ownership-type r))
        (cons "active" (activeflag r))))
