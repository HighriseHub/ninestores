...
DROP TABLE IF EXISTS DOD_PASSWORD_RSET; SELECT ' A table to reset password for customers, vendors and employees'; 
DROP TABLE IF EXISTS DOD_VENDOR_APPOINTMENT; SELECT 'A table for vendor appointments'; 
DROP TABLE IF EXISTS DOD_VENDOR_AVAILABILITY; SELECT 'A table for vendor availability';
...
DROP TABLE IF EXISTS DOD_BUS_ADAPTER; SELECT 'A table to store the hhub Business Adapter configurations';
...
CREATE TABLE DOD_COMPANY( ROW_ID MEDIUMINT AUTO_INCREMENT     PRIMARY KEY ,
 ...
 CREATED TIMESTAMP NOT NULL DEFAULT NOW() ,
 WEBSITE VARCHAR(256),
 CMP_TYPE VARCHAR(30) NOT NULL DEFAULT "DEFAULT", 'other types are GROCERY, APPAREL, MOBILE, ELECTRONICS, HYPERMARKET, FASHION JEWELLERY, OTHER'
  CREATED_BY MEDIUMINT,
  UPDATED TIMESTAMP ,
  UPDATED_BY MEDIUMINT,
...
BILLSAMEASSHIP CHAR(1),
GSTNUMBER VARCHAR(20),
GSTORGNAME VARCHAR(50),
...
CREATED TIMESTAMP NOT NULL DEFAULT NOW() ,
TENANT_ID MEDIUMINT,
ORDER_TYPE CHAR(4) DEFAULT "SALE",
DELETED_STATE CHAR(1));
...
SGST DECIMAL(2,2), 
IGST DECIMAL(2,2),
DISC_RATE DECIMAL(2,2),
ADDL_TAX1_RATE DECIMAL(2,2), 'EXAMPLE- CESS'
FULFILLED CHAR(1),
STATUS CHAR(3), 
CREATED TIMESTAMP NOT NULL DEFAULT NOW() ,
DELETED_STATE CHAR(1),
...
PRD_IMAGE_PATH1 VARCHAR(256),
PRD_IMAGE_PATH2 VARCHAR(256),
PRD_IMAGE_PATH3 VARCHAR(256),
PRD_IMAGE_PATH4 VARCHAR(256),
HSN_CODE VARCHAR(8), 
...
APPROVAL_STATUS VARCHAR(20), 
APPROVED_BY VARCHAR(30),
PRD_TYPE CHAR(6) DEFAULT "GOODS",
TENANT_ID MEDIUMINT 
);


SELECT 'CREATING TABLE DOD_ORDER_SHIPMENT' AS ' ';
CREATE TABLE DOD_ORDER_SHIPMENT (ROW_ID MEDIUMINT AUTO_INCREMENT PRIMARY KEY,
CUST_ORDER_ID MEDIUMINT, 
VENDOR_ORDER_ID MEDIUMINT,
...
PHONE VARCHAR(30) NOT NULL, 
 ...
 PAYMENT_API_KEY VARCHAR(40), 
 PAYMENT_API_SALT VARCHAR(40), 
 CREATED TIMESTAMP NOT NULL DEFAULT NOW() ,
 UPDATED TIMESTAMP , 
 DELETED_STATE CHAR(1),
 APPROVED_FLAG CHAR(1), 
...
CREATE TABLE DOD_UPI_PAYMENTS (ROW_ID MEDIUMINT AUTO_INCREMENT PRIMARY KEY, 
TRANSACTION_ID VARCHAR(20), 
VENDOR_ID MEDIUMINT NOT NULL,
CUST_ID MEDIUMINT NOT NULL,
AMOUNT DECIMAL(9,2) NOT NULL,
STATUS CHAR(10), 'PENDING, FAILED, ERROR, SUCCESS '
UTRNUM VARCHAR(20) NOT NULL,
VENDORCONFIRM CHAR(1), 
'TOMBSTONE FIELDS' 
CREATED TIMESTAMP NOT NULL DEFAULT NOW(),
UPDATED TIMESTAMP,
CREATED_BY MEDIUMINT,
...
CREATE TABLE DOD_PASSWORD_RESET(ROW_ID MEDIUMINT AUTO_INCREMENT PRIMARY KEY, 
...
CREATE TABLE DOD_ABAC_SUBJECT (ROW_ID MEDIUMINT AUTO_INCREMENT PRIMARY KEY, 
NAME VARCHAR(100),
HHUB_TYPE VARCHAR(100), 
CREATED TIMESTAMP NOT NULL DEFAULT NOW() ,
CREATED_BY MEDIUMINT,
...
NAME VARCHAR(100), 
URI VARCHAR(100), 
TRANSACTION_FUNC VARCHAR(100), 
AUTH_POLICY_ID MEDIUMINT NOT NULL, 
ABAC_SUBJECT_ID MEDIUMINT NOT NULL, 
...
CREATE TABLE DOD_VENDOR_AVAILABILITY_DAY (ROW_ID MEDIUMINT AUTO_INCREMENT PRIMARY KEY, 
VENDOR_ID MEDIUMINT NOT NULL, 
...
SELECT 'CREATING TABLE DOD_VENDOR_AVAILABILITY' AS ' '; 
CREATE TABLE DOD_VENDOR_APPOINTMENT (ROW_ID MEDIUMINT AUTO_INCREMENT PRIMARY KEY, 
VENDOR_ID MEDIUMINT NOT NULL, 
CUSTOMER_ID MEDIUMINT,  -- May be null as it could be a guest customer. 
...
CREATE TABLE DOD_WEBPUSH_NOTIFY  (ROW_ID MEDIUMINT AUTO_INCREMENT PRIMARY KEY, 
CUST_ID MEDIUMINT, 
VENDOR_ID MEDIUMINT,
...
SELECT 'CREATING TABLE DOD_BUS_ADAPTER' AS ' ';
CREATE TABLE DOD_BUSINESS_OBJECT_ADAPTER (ROW_ID MEDIUMINT AUTO_INCREMENT PRIMARY KEY,
NAME VARCHAR(255),
DESCRIPTION VARCHAR(255),
BO_ID MEDIUMINT,
BO_METHOD_NAME,
...
-- CREATE FOREIGN KEY REFERENCES : DOD_BUS_OBJECT TABLE
SELECT 'CREATE FOREIGN KEY REFERENCES : DOD_ABAC_SUBJECT TABLE' AS ' ';
ALTER TABLE DOD_ABAC_SUBJECT ADD FOREIGN KEY(TENANT_ID) REFERENCES DOD_COMPANY(ROW_ID);
...
ALTER TABLE DOD_BUS_TRANSACTION ADD FOREIGN KEY(ABAC_SUBJECT_ID) REFERENCES DOD_ABAC_SUBJECT(ROW_ID); 
...
-- CREATE FOREIGN KEY REFERENCES : DOD_PASSWORD_RESET
SELECT 'CREATE FOREIGN KEY REFERENCES : DOD_USERS TABLE' AS ' '; 
ALTER TABLE DOD_PASSWORD_RESET ADD FOREIGN KEY (TENANT_ID) REFERENCES DOD_COMPANY(ROW_ID); 



-- ALTER TABLE DOD_CUST_PROFILE ADD UNIQUE KEY
ALTER IGNORE TABLE DOD_CUST_PROFILE  ADD CONSTRAINT UC_Customer UNIQUE (PHONE, TENANT_ID, CUST_TYPE);
...
-- CREATE FOREIGN KEY REFERENCES : DOD_VENDOR_AVAILABILITY_DAY
SELECT 'CREATE FOREIGN KEY REFERENCES : DOD_VENDOR_AVAILABILITY_DAY TABLE' AS ' '; 
ALTER TABLE DOD_VENDOR_AVAILABILITY_DAY  ADD FOREIGN KEY (TENANT_ID) REFERENCES DOD_COMPANY(ROW_ID); 
ALTER TABLE DOD_VENDOR_AVAILABILITY_DAY  ADD FOREIGN KEY (VENDOR_ID)   REFERENCES DOD_VEND_PROFILE(ROW_ID);


-- CREATE FOREIGN KEY REFERENCES : DOD_VENDOR_APPOINTMENT
SELECT 'CREATE FOREIGN KEY REFERENCES : DOD_VENDOR_APPOINTMENT TABLE' AS ' '; 
ALTER TABLE DOD_VENDOR_APPOINTMENT  ADD FOREIGN KEY (TENANT_ID) REFERENCES DOD_COMPANY(ROW_ID); 
ALTER TABLE DOD_VENDOR_APPOINTMENT  ADD FOREIGN KEY (VENDOR_ID)   REFERENCES DOD_VEND_PROFILE(ROW_ID);
ALTER TABLE DOD_VENDOR_APPOINTMENT  ADD FOREIGN KEY (CUST_ID)   REFERENCES DOD_CUST_PROFILE(ROW_ID);

-- CREATE FOREIGN KEY REFERENCES : DOD_WEBPUSH_NOTIFY
SELECT 'CREATE FOREIGN KEY REFERENCES : DOD_WEBPUSH_NOTIFY TABLE' AS ' '; 
ALTER TABLE DOD_WEBPUSH_NOTIFY  ADD FOREIGN KEY (TENANT_ID) REFERENCES DOD_COMPANY(ROW_ID); 
ALTER TABLE DOD_WEBPUSH_NOTIFY  ADD FOREIGN KEY (VENDOR_ID)   REFERENCES DOD_VEND_PROFILE(ROW_ID);
ALTER TABLE DOD_WEBPUSH_NOTIFY  ADD FOREIGN KEY (CUST_ID)   REFERENCES DOD_CUST_PROFILE(ROW_ID);
...
SELECT 'CREATE demoadmin  USER' AS ' ';
INSERT INTO DOD_USERS(NAME ,
 USERNAME ,
 PASSWORD ,
 CREATED ,
 ...
 TENANT_ID, 
 ACTIVE_FLG,
 DELETED_STATE ) VALUES("Demo Admin", "demoadmin", "P@ssword1", CURDATE(), -1, CURDATE(), -1, 2,'Y','N'); 
...
SELECT 'GIVE operator1 USER A operator ROLE' AS ' '; 
INSERT INTO DOD_USER_ROLES (USER_ID, ROLE_ID, TENANT_ID) VALUES
(2, 2, 1); 


-- GIVE demoadmin  USER A COMPADMIN  ROLE
SELECT 'GIVE DEMOADMIN USER A COMPADMIN ROLE' AS ' '; 
INSERT INTO DOD_USER_ROLES (USER_ID, ROLE_ID, TENANT_ID) VALUES
(3, 3, 2); 
...
INSERT INTO DOD_BUS_TRANSACTION (NAME,URI, AUTH_POLICY_ID, TRANS_TYPE, ACTIVE_FLG, DELETED_STATE, TENANT_ID) VALUES
("CREATE COMPANY", "/new-company", 1, "CREATE", "Y", "N", 1); 
